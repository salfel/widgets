...
main :: proc() {
	...
	box_set_property(button, .Expand_Horizontal, &app_context.renderer)
	...
	box_style_set_height(child2, 300, &app_context.renderer)
	box_style_set_background(child2, RED, &app_context.renderer)
	box_style_set_margin(child2, sides_make(20, 20, 0, 0), &app_context.renderer)
	...
	box_set_property(child2, .Expand_Horizontal, &app_context.renderer)

	child3 := box_make()
	widget_register(child3, &app_context.widget_manager)
	widget_add_child(parent, child3)
	box_style_set_height(child3, 300, &app_context.renderer)
